...
- **border-radius:** `rounded-full` (9999px). Buttons are pill-shaped. Applies to both variants and all sizes including icon-only.
- **icon library:** `@tabler/icons-vue`
- **icon import pattern:** `import { IconSun } from '@tabler/icons-vue'`
- **icon sizing:** always `class="h-4 w-4"` inside the default slot
- **icon-only shape:** because padding is square (`p-2.5`) and border-radius is `rounded-full`, icon-only buttons render as circles.
## behavior
...
- Buttons are pill-shaped (`rounded-full`). Never use a smaller border-radius — this is a core visual identity decision, not a default.
- Icon-only buttons are circles. Do not adjust width/height to make them non-circular.
- Always use `@tabler/icons-vue` for icons. Never use inline SVG or a different icon library unless explicitly told the project has changed its icon dependency.
## examples
...
<Button icon-only aria-label="Toggle theme">
